...
const STEM_COLORS: Record<StemType, { border: string; text: string; bg: string }> = {
  vocals: { border: 'border-cyan-500/30', text: 'text-cyan-400', bg: 'bg-cyan-500/10' },
  drums: { border: 'border-orange-500/30', text: 'text-orange-400', bg: 'bg-orange-500/10' },
  bass: { border: 'border-purple-500/30', text: 'text-purple-400', bg: 'bg-purple-500/10' },
  other: { border: 'border-emerald-500/30', text: 'text-emerald-400', bg: 'bg-emerald-500/10' },
};
...
export const FxRackView: React.FC<FxRackViewProps> = ({
  ...
  return (
    <div className={`flex flex-col h-full bg-[#0d0e12] select-none text-zinc-300 font-mono ${className}`}>
      {/* Header & Stem Tabs */}
      <div className="h-14 px-5 bg-[#14151a] border-b border-[#252832] flex items-center justify-between shrink-0">
        <div className="flex items-center space-x-3">
          <Sliders className="w-5 h-5 text-cyan-400" />
          <h2 className="font-bold text-sm tracking-wider uppercase text-zinc-100">
            Stem Inserts &amp; FX Rack
          </h2>
          <span className="text-[10px] text-zinc-500 uppercase px-2 py-0.5 bg-[#1b1c24] rounded border border-[#2a2d38]">
            32-Bit Float DSP
          </span>
        </div>

        {/* Stem Switcher Buttons */}
        <div className="flex items-center space-x-1.5 bg-[#0a0a0d] p-1 rounded-lg border border-[#23252f]">
          {STEM_TYPES.map((stem) => {
            ...
            return (
              <button
                ...
                className={`flex items-center space-x-2 px-3 py-1.5 rounded-md text-xs font-bold transition-all ${
                  isSelected
                    ? `${STEM_COLORS[stem].bg} ${STEM_COLORS[stem].text} border ${STEM_COLORS[stem].border} shadow-sm`
        ...
        <button
          onClick={resetStemFx}
          className="flex items-center space-x-1 px-2.5 py-1.5 bg-[#1a1b22] hover:bg-[#252834] text-zinc-400 hover:text-white rounded border border-[#262832] text-xs transition-all"
          title="Reset current stem effects to defaults"
        >
          <RotateCcw className="w-3.5 h-3.5" />
          <span className="text-[11px]">Reset FX</span>
        </button>
      </div>

      {/* Main FX Insert Grid */}
      <div className="flex-1 p-5 overflow-y-auto grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-5 gap-4">
        {/* 1. 3-BAND EQ */}
        <div
          className={`bg-[#121318] border rounded-xl p-4 flex flex-col justify-between shadow-lg transition-all ${
            currentFx.eq.enabled ? 'border-cyan-500/40 shadow-[0_0_15px_rgba(6,182,212,0.1)]' : 'border-[#242630] opacity-75'
          }`}
        >
          <div>
            <div className="flex items-center justify-between border-b border-[#22242e] pb-2.5 mb-3">
              <div className="flex items-center space-x-2">
                <Activity className="w-4 h-4 text-cyan-400" />
                <span className="font-bold text-xs uppercase text-zinc-100 tracking-wider">3-Band EQ</span>
              </div>
              <button
                onClick={() =>
                  updateStemFx((prev) => ({
                ...
                className={`p-1 rounded transition-all ${
                  currentFx.eq.enabled
                    ...
                    : 'bg-[#1b1c24] text-zinc-600 border border-[#262832]'
                }`}
                title="Bypass EQ"
              >
            ...
            <div className="flex flex-col gap-3 text-[11px]">
              <div>
                ...
                <input
                  ...
                  className="w-full accent-cyan-400 h-1 bg-[#22242e] rounded cursor-pointer"
                />
              </div>
          ...
          <div className="mt-4 pt-2 border-t border-[#22242e] flex justify-between text-[10px] text-zinc-500">
            <span>Low: 100Hz</span>
            <span>Mid: 1.0k</span>
            <span>Hi: 8.0k</span>
          </div>
        ...
        >
          <div>
            <div className="flex items-center justify-between border-b border-[#22242e] pb-2.5 mb-3">
              <div className="flex items-center space-x-2">
                ...
                <span className="font-bold text-xs uppercase text-zinc-100 tracking-wider">Compressor</span>
              </div>
              <button
                onClick={() =>
                  updateStemFx((prev) => ({
            ...
            <div className="flex flex-col gap-3 text-[11px]">
              <div>
                ...
                <input
                  ...
                  className="w-full accent-orange-400 h-1 bg-[#22242e] rounded cursor-pointer"
                />
              </div>
          ...
          <div className="mt-4 pt-2 border-t border-[#22242e]">
        ...
        >
          <div>
            <div className="flex items-center justify-between border-b border-[#22242e] pb-2.5 mb-3">
              <div className="flex items-center space-x-2">
                ...
                <span className="font-bold text-xs uppercase text-zinc-100 tracking-wider">Convolver Reverb</span>
              </div>
              <button
                onClick={() =>
                  updateStemFx((prev) => ({
            ...
            <div className="flex flex-col gap-3 text-[11px]">
              <div>
                ...
                <input
                  ...
                  className="w-full accent-purple-400 h-1 bg-[#22242e] rounded cursor-pointer"
                />
              </div>
              ...
              <div className="flex items-center space-x-1 pt-1">
                {[
                  ...
                  <button
                    ...
                    className="flex-1 py-1 bg-[#1b1c24] hover:bg-[#262836] text-[10px] rounded border border-[#2b2d38] text-zinc-400 hover:text-white"
                  >
                    {p.name}
                  </button>
                ))}
          ...
          <div className="mt-4 pt-2 border-t border-[#22242e] text-[10px] text-zinc-500">
          ...
          <div>
            <div className="flex items-center justify-between border-b border-[#22242e] pb-2.5 mb-3">
              <div className="flex items-center space-x-2">
                ...
                <span className="font-bold text-xs uppercase text-zinc-100 tracking-wider">Stereo Delay</span>
              </div>
              <button
                onClick={() =>
                  updateStemFx((prev) => ({
            ...
            <div className="flex flex-col gap-3 text-[11px]">
              <div>
                ...
                <input
                  ...
                  className="w-full accent-emerald-400 h-1 bg-[#22242e] rounded cursor-pointer"
                />
              </div>
          ...
          <div>
            <div className="flex items-center justify-between border-b border-[#22242e] pb-2.5 mb-3">
              <div className="flex items-center space-x-2">
                ...
                <span className="font-bold text-xs uppercase text-zinc-100 tracking-wider">Saturation</span>
              </div>
              <button
                onClick={() =>
                  updateStemFx((prev) => ({
            ...
            <div className="flex flex-col gap-3 text-[11px]">
              <div>
                ...
                <input
                  ...
                  className="w-full accent-pink-400 h-1 bg-[#22242e] rounded cursor-pointer"
                />
              </div>
